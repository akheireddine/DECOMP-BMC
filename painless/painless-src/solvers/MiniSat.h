...
public:
   ...
   void createNewVar(bool polarity, bool dvar);

   void set_simplification(bool value);

   void simplificate();
   
   void getClauseDatabase(std::vector< ClauseExchange* > & clausesDB);

   std::vector<int> getAssignments();

   Minisat::SimpSolver * getSolver(){ return solver; };
...
protected:
   ...
   friend bool minisatImportClause(void *, Minisat::vec<Minisat::Lit, int> &, int &);
};
