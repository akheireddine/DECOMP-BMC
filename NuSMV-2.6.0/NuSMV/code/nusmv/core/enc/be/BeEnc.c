...
{
  be_ptr res, renaming, expr_timed;
  ...
  if (from > to) {
    ...
    if(step_phy_index < 0)
      expr_timed = BeEnc_untimed_expr_to_timed(self, exp, to);
    else{
      renaming = Be_Index2Var(self->be_mgr, to + step_phy_index);
      // expr_timed = Be_And(self->be_mgr, renaming, Be_Iff(self->be_mgr, renaming, BeEnc_untimed_expr_to_timed(self, exp, to)));
      expr_timed = Be_Iff(self->be_mgr, renaming, BeEnc_untimed_expr_to_timed(self, exp, to));
    }
    res =  Be_And(self->be_mgr,
                  BeEnc_untimed_to_timed_and_interval(self, exp, from, to-1, step_phy_index),
                  expr_timed);
  }
...
int get_max_used_phy_idx(BeEnc_ptr self){
  return self->max_used_phy_idx;
}
...
*/
void be_enc_allocate_space_for_new_vars(BeEnc_ptr self, int vars_num)
{
