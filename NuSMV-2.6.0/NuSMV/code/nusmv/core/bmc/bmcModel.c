...
be_ptr Bmc_Model_GetInit0(const BeFsm_ptr be_fsm, const int step_phy_index)
{
  BeEnc_ptr be_enc = BeFsm_get_be_encoding(be_fsm);
  Be_Manager_ptr be_mgr = BeEnc_get_be_manager(be_enc);
  be_ptr renaming, init0_rename;
  ...
  renaming = Be_Index2Var(be_mgr, step_phy_index);

  init0_rename = Be_Iff(be_mgr, renaming, init0);
  // init0_rename = Be_And(be_mgr, renaming, Be_Iff(be_mgr, renaming, init0));
...
be_ptr
Bmc_Model_GetUnrolling(const BeFsm_ptr be_fsm, const int j, const int k, const int step_phy_index)
{
  BeEnc_ptr be_enc = BeFsm_get_be_encoding(be_fsm);
  Be_Manager_ptr be_mgr = BeEnc_get_be_manager(be_enc);
  ...
  return BeEnc_untimed_to_timed_and_interval(be_enc, trans_invar, j, k - 1, step_phy_index);
}
...
be_ptr Bmc_Model_GetPathNoInit(const BeFsm_ptr be_fsm, const int k, const int step_phy_index)
{
  return Bmc_Model_GetUnrolling(be_fsm, 0, k, step_phy_index);
}

be_ptr Bmc_Model_GetPathWithInit(const BeFsm_ptr be_fsm, const int k, int * step_phy_index)
{
  BeEnc_ptr be_enc = BeFsm_get_be_encoding(be_fsm);
  *step_phy_index = get_max_used_phy_idx(be_enc) + 1;
  for(int i = 0; i < k; i++){
    be_enc_allocate_space_for_new_vars(be_enc, k+i + *step_phy_index);
    be_enc_create_be_var(be_enc, k + *step_phy_index, NULL);
  }

  return Be_And(BeEnc_get_be_manager(BeFsm_get_be_encoding(be_fsm)),
                Bmc_Model_GetPathNoInit(be_fsm, k, *step_phy_index),
                Bmc_Model_GetInit0(be_fsm, *step_phy_index));
}
