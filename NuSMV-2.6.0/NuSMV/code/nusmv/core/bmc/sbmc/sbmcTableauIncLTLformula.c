          ...
          if (do_optimization) {
            ...
            if (sbmc_node_info_get_trans_vars(info) == (array_t *)NULL) {
              ...
              for (d = 0; d <= sbmc_node_info_get_past_depth(info); d++) {
                ...
                const unsigned int d_lsf = min_nusmv(d, sbmc_node_info_get_past_depth(lsf_info));
                const unsigned int d_rsf = min_nusmv(d, sbmc_node_info_get_past_depth(rsf_info));
        ...
        if (do_optimization) {
          ...
          for (d = 0; d <= sbmc_node_info_get_past_depth(info); d++) {
            ...
            be_aux_next = array_fetch(be_ptr, L_past_array,
                                      min_nusmv(d+1, sbmc_node_info_get_past_depth(info)));
        ...
        if (do_optimization) {
          ...
          for (d = 0; d <= sbmc_node_info_get_past_depth(info); d++) {
            ...
            be_f_E_d = array_fetch(be_ptr, past_array_f_E,
                                   min_nusmv(d, sbmc_node_info_get_past_depth(lsf_info)));
            nusmv_assert((be_ptr)NULL != be_f_E_d);

            be_g_E_d = array_fetch(be_ptr, past_array_g_E,
                                   min_nusmv(d, sbmc_node_info_get_past_depth(rsf_info)));
            nusmv_assert((be_ptr)NULL != be_g_E_d);
            ...
            if (opt_verbose_level_ge(opts, 6)) {
              ...
              Logger_log(logger, "  [[fUg]]_E^%u <=> [[g]]_E^%u | ([[f]]_E^%u & [[fUg]]_L^min(%u,%u)): ",
                      d, min_nusmv(d, sbmc_node_info_get_past_depth(rsf_info)),
                      min_nusmv(d, sbmc_node_info_get_past_depth(lsf_info)),
      ...
      for (d = 0; d <= sbmc_node_info_get_past_depth(info); d++) {
        ...
        if (opt_verbose_level_ge(opts, 6)) {
          ...
          Logger_log(logger, "  ([[f]]_%d^%d <=> [[f]]_L^%d): ",
                  k_model+1, d, min_nusmv(d+1,sbmc_node_info_get_past_depth(info)));
          Be_DumpSexpr(be_mgr, be_result, errstream);
          Logger_log(logger, "\n");
        }
      }
                                    ...
                                    const int opt_do_optimization)
{
  ...
  for (i = max_nusmv(previous_k + 1, 0); i <= new_k; i++) {
