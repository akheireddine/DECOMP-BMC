                    ...
                    const char* dump_fname_template)
{
  ...
  for (increasingK = k_min; (increasingK <= k_max) && ! found_solution;
    ...
    prob = Bmc_Gen_LtlProblem(be_fsm, bltlspec, increasingK, l, -1);
    ...
    if (dump_type != BMC_DUMP_NONE) {
      ...
      Bmc_Dump_WriteProblem(be_enc, cnf, ltlprop, increasingK, l, -1,
                                       ...
                                       Trace_ptr* trace)
{
  ...
  be_init = Bmc_Model_GetInit0(be_fsm, -1);
  ...
  while (!solved && (k <= max_k)) {
    ...
    be_base = Bmc_Model_GetUnrolling(be_fsm, 0, k, -1);
    ...
    if (!solved) {
      ...
      */
      ...
      /* Get the unrolling (s_0,...,s_k)*/
      be_steps[0] = Bmc_Model_GetUnrolling(be_fsm, 0, k, -1);
