...
static int
bmc_dump_openDimacsFile2(const NuSMVEnv_ptr env,
                        const char* filename, const int write_property,
                        FILE** file_ref);
                           ...
                           const char* dump_fname_template)
{
  ...
  case BMC_DUMP_DIMACS:
    ...
    if (Prop_get_type(prop) == Prop_Invar) {
      ...
      Bmc_Dump_DimacsProblemFilename(be_enc, cnf, dumpFilenameExpanded, k, step_phy_index);
    }
    break;
...
void Bmc_Dump_WriteProblem2(const BeEnc_ptr be_enc,
                           const Be_Cnf_ptr cnf,
                           Prop_ptr prop,
                           const int k, const int loop,
                           const Bmc_DumpType dump_type,
                           const char* dump_fname_template,
                           const int write_property,
                           const int * property_bool)
{
  const NuSMVEnv_ptr env = EnvObject_get_environment(ENV_OBJECT(be_enc));
  const StreamMgr_ptr streams =
    STREAM_MGR(NuSMVEnv_get_value(env, ENV_STREAM_MANAGER));
  ...
  case BMC_DUMP_DIMACS:
    ...
    if (Prop_get_type(prop) == Prop_Invar) {
      ...
      Bmc_Dump_DimacsProblemFilename2(be_enc, cnf, dumpFilenameExpanded, k, write_property, property_bool);
    }
    break;
                                   ...
                                   const int step_phy_index)
{
  ...
  if (ret == 0) {
    Bmc_Dump_DimacsProblem(be_enc, cnf, k, step_phy_index, file);
    fclose(file);
  }
  return ret;
}

int Bmc_Dump_DimacsProblemFilename2(const BeEnc_ptr be_enc,
                                   const Be_Cnf_ptr cnf,
                                   const char* filename,
                                   const int k, const int write_property,
                                   const int * property_bool)
{
  const NuSMVEnv_ptr env = EnvObject_get_environment(ENV_OBJECT(be_enc));

  FILE* file;
  int ret = bmc_dump_openDimacsFile2(env, filename, write_property, &file);

  if (ret == 0) {
    Bmc_Dump_DimacsProblem2(be_enc, cnf, k, file, write_property, property_bool);
    fclose(file);
  }
  return ret;
}
...
void Bmc_Dump_DimacsInvarProblem(const BeEnc_ptr be_enc,
                                 ...
                                 FILE* dimacsfile)
{
  Bmc_Dump_DimacsProblem(be_enc, cnf, 1, -1, dimacsfile);
}
                            ...
                            FILE* dimacsfile)
{
  ...
  loop_vars = get_max_used_phy_idx(be_enc) + 1;
  for(int loop_index = loop_vars - k; loop_index < loop_vars; loop_index++){
    int cnf_index = Be_BeIndex2CnfLiteral(be_mgr, loop_index);
    fprintf(dimacsfile, "c LOOP variable %d => Size %d, Definition\n",
                                        cnf_index,loop_index-(loop_vars - k));
  }
  if(step_phy_index>-1){
    for(int step_id = step_phy_index, i = 0; step_id < step_phy_index+k; step_id++,i++){
        int cnf_index = Be_BeIndex2CnfLiteral(be_mgr, step_id);
        fprintf(dimacsfile, "c STEP variable %d => Unrolling %d, Renaming\n",
                                              cnf_index,i);
    }
  }
  ...
  /* Actually writes the dimacs data: */
  {
    ...
    if(step_phy_index>-1){// IMPORTANT CUZ (renaming) clause not added before in Be_enc
      for(int step_id = step_phy_index; step_id < step_phy_index+k; step_id++){
          int cnf_index = Be_BeIndex2CnfLiteral(be_mgr, step_id);
          fprintf(dimacsfile, "%d 0\n",cnf_index);
      }
    }
...
void Bmc_Dump_DimacsProblem2(const BeEnc_ptr be_enc,
                            const Be_Cnf_ptr cnf,
                            const int k,
                            FILE* dimacsfile, const int write_property,
                            const int * property_bool)
{
  const NuSMVEnv_ptr env = EnvObject_get_environment(ENV_OBJECT(be_enc));
  const OptsHandler_ptr opts =
    ...
    if (Be_Cnf_GetFormulaLiteral(cnf) == INT_MAX) {
      ...
      SLIST_FOREACH(Be_Cnf_GetClausesList(cnf), genCl) {
        ...
        while (cl[i] != 0) {
          ...
          fprintf(dimacsfile, "%" PRIdPTR " ", lit);
          i++;
          if( property_bool != NULL && property_bool[abs(lit)])
            property_on = 1;
        }
        if(property_on==1)
          fprintf(dimacsfile, "-1 0\n");
...
static int bmc_dump_openDimacsFile2(const NuSMVEnv_ptr env,
                                   const char* filename, const int write_property,
                                   FILE** file_ref)
{
  const StreamMgr_ptr streams =
