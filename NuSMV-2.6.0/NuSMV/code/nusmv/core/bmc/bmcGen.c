                          ...
                          int *step_phy_index)
{
  ...
  be_ptr path_k = Bmc_Model_GetPathWithInit(be_fsm, k, step_phy_index);
                               ...
                               const node_ptr wff)
{
  ...
  be_ptr trans_01_invar_01 = Bmc_Model_GetPathNoInit(be_fsm, 1, -1);
...
be_ptr Bmc_Gen_UnrollingFragment(BeFsm_ptr be_fsm, const int i)
{
  ...
  /* Init[0] & Invar[0] */
  if (0 == i) {
    return Bmc_Model_GetInit0(be_fsm, -1);
  }
