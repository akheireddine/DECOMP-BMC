                       ...
                       const boolean must_inc_length)
{
  ...
  beInit = Bmc_Model_GetInit0(be_fsm,-1);
  ...
  /* start of ZifZag algorithm */
  {
    ...
    for (stepN=0; stepN <= max_k; ++stepN) {
      ...
      { /* Insert transition relation (stepN,stepN+1) permanently */
        be_ptr unrolling = Bmc_Model_GetUnrolling(be_fsm, stepN, stepN+1, -1);
                          ...
                          bmc_invar_closure_strategy strategy)
{
  ...
  /* start of Dual algorithm */
  {
    ...
    bmc_add_be_into_solver_positively(
        ...
        Bmc_Model_GetInit0(be_fsm, -1), be_enc, cnf_alg);
    ...
    for (stepN=0; stepN <= max_k; ++stepN) {
      ...
      /* Insert transition relation (stepN,stepN+1) permanently */
      unrolling = Bmc_Model_GetUnrolling(be_fsm, stepN, stepN+1, -1);
                                   ...
                                   const int max_k, int step_k)
{
  ...
  /* start of Falsification algorithm */
  {
    ...
    for (stepN=0; stepN <= max_k; stepN += step_k) {
      ...
      if (limit_k - stepN > 1) {
        be_ptr unrolling =
          Bmc_Model_GetUnrolling(be_fsm, stepN, stepN + limit_k - 1, -1);
...

      {
        ...
        be_ptr unrolling = Bmc_Model_GetUnrolling(be_fsm, limit_k-1, limit_k, -1);
